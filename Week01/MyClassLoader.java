package org.example;

import java.lang.reflect.Method;
import java.util.Base64;

public class MyClassLoader extends ClassLoader{
    public static void main(String[] args) throws Exception{
        Class hello1 = new MyClassLoader().findClass("Hello");
        Object o = hello1.newInstance();
        for (Method method : hello1.getMethods()) {
            if ("hello".equals(method.getName())) {
                System.out.println("调用hello方法");
                method.invoke(o);
            }
        }


    }


    @Override
    protected Class<?> findClass(String name) throws ClassNotFoundException {
       String calssSTR = "";
        byte[] decode = Base64.getDecoder().decode(calssSTR);
        byte[] bytes =new byte[decode.length];
        for (int i = 0; i < decode.length; i++) {
            Byte b = decode[i];
            Byte b1 = (byte)(-(b -255)) ;
            bytes[i] = b1;
        }
        return defineClass(name,bytes,0,bytes.length);

    }
}
